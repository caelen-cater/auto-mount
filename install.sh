...
#
# Auto-Mount SFTP Service v1.2.1
...
main() {
    ...
    # Always attempt to mount (it's safe to mount even if already mounted)
...
# Function to repair/regenerate files for existing servers
repair_server() {
    print_header "Repair Server Files"
    echo
    ...
    if [[ ! -d "$CONFIG_DIR" ]] || [[ -z "$(ls -A "$CONFIG_DIR" 2>/dev/null)" ]]; then
        print_warning "No servers to repair"
        return 0
    fi
    
    read -p "Enter server name to repair (or 'all' for all servers): " server_name
    ...
    if [[ "$server_name" == "all" ]]; then
        print_status "Repairing all servers..."
        local count=0
        for config_file in "$CONFIG_DIR"/*.conf; do
            if [[ -f "$config_file" ]]; then
                local current_server=$(basename "$config_file" .conf)
                print_status "Repairing server: $current_server"
                repair_single_server "$current_server"
                ((count++))
            fi
        done
        
        if [[ $count -eq 0 ]]; then
            print_warning "No servers found to repair"
        else
            print_status "Repaired $count server(s) successfully!"
        fi
    else
        if [[ ! -f "$CONFIG_DIR/$server_name.conf" ]]; then
            print_error "Server '$server_name' not found"
        ...
        print_status "Repairing server '$server_name'..."
        repair_single_server "$server_name"
        print_status "Server '$server_name' repaired successfully!"
    fi
}

# Function to repair a single server
repair_single_server() {
    local server_name="$1"
    
    # Load existing configuration
    source "$CONFIG_DIR/$server_name.conf"
    
    print_status "Regenerating files for '$server_name'..."
    ...
    print_status "Files regenerated for '$server_name'"
    echo "  Script: $BIN_DIR/auto-mount-$server_name.sh"
    echo "  Service: $SERVICE_DIR/auto-mount-$server_name.service"
    echo "  Cron: $CRON_DIR/auto-mount-$server_name"
    echo "  Log: $LOG_DIR/$server_name.log"
}
...
show_help() {
    ...
    echo "  repair    Repair/regenerate files for existing servers"
    ...
    echo "  $0 repair       # Repair/regenerate files"
